...
int main( int argc, char *argv[] )  
{
  ...
  // load the weights
    int8_t  filter1[NUM_FILTERS_1][NUM_channel_01][FILTER_SIZE1][FILTER_SIZE1];
  ...
  // load the input feature map
    ...
    uint8_t  ifmap0 [IMAGE_WIDTH][IMAGE_HEIGHT][NUM_channel_01];
    const char* ifmap_1 = "ifmap_L1.txt";

    load_matrix_if1(ifmap0, ifmap_1);  //loading


    uint8_t  ifmap11 [IMAGE_WIDTH+ 2 * PAD_SIZE][IMAGE_HEIGHT+ 2 * PAD_SIZE][NUM_channel_01];

     padding_L1(ifmap0, ifmap11);   /// padding
...
uint8_t    output1[IMAGE_WIDTH-FILTER_SIZE1+1+ 2 * PAD_SIZE][IMAGE_WIDTH-FILTER_SIZE1+1+ 2 * PAD_SIZE][NUM_FILTERS_1];
    ...
    conv2d_11(ifmap11, filter1, output1);
